  ...
  >
    <div class="c-masterpiece__inner">
      <div>
        ...
        <h4>{{ masterpiece.author }}, {{ masterpiece.creationYear }}</h4>
      </div>

      <div v-if="masterpiece.isSold" class="c-masterpiece__control">
        <c-text-button doublet>
          <c-icon icon="sold" />
          Продана на аукционе
        </c-text-button>
      </div>
      <div v-else class="c-masterpiece__control">
        <div>
          <div
            v-if="masterpiece.discountedPrice"
        ...
        <c-button v-else @click="() => addProduct(masterpiece)">
          Купить
        </c-button>
      </div>
    </div>
